...
export const SlotPairingModal: React.FC<SlotPairingModalProps> = ({ isOpen, onClose, slotLabel, availableStudents }) => {
  ...
  return (
    <div className="app-modal-backdrop items-end justify-center sm:items-center" role="presentation" onClick={onClose}>
      <section className="app-modal max-h-[90vh] max-w-3xl overflow-y-auto" role="dialog" aria-modal="true" aria-labelledby="slot-pairing-title" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="mt-5 flex flex-col gap-2 border-t border-border pt-4 sm:flex-row sm:items-center sm:justify-between"><span className="text-xs text-muted">{copied ? 'Copied to clipboard.' : `${validPairs.length} group(s) ready`}</span><div className="flex gap-2"><button type="button" onClick={onClose} className="app-button-secondary">Close</button><button type="button" onClick={() => void copyWhatsApp()} disabled={!validPairs.length} className="app-button-primary">{copied ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}{copied ? 'Copied' : 'Copy Pairs for WhatsApp'}</button></div></div>
      </section>
    </div>
  );
};